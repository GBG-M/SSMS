...
export default function ParentRoute({ children }) {
  ...
  useEffect(() => {
    async function checkParentRole() {
      if (!isAuthenticated()) {
      ...
      try {
        ...
        console.error("Failed to verify parent role:", error);
        setHasAccess(false);
      } finally {
        setCheckingRole(false);
      }
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-slate-50">
        <div className="text-center">
          ...
          <p className="text-sm font-medium text-slate-600">Verifying Guardian Access...</p>
        </div>
      </div>
    );
  }
